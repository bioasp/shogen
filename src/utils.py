...
def readcouples(couple_string, dictr, revdictr) :
  ...
  for line in bla :
    link = line.split()
    if link[0] not in dictr :# Dictionnary for the reactions
      r+=1
      dictr[link[0]] = r
      revdictr.append(link[0])
    if link[1] not in dictr :
...
def print_reactions(answer,dictr,dictg) :
  for a in answer:
    if a.pred() == "fedge" :
      print("\n    ",dictr[int(a.arg(0).arg(1))],"(",dictg[int(a.arg(0).arg(0))],")",sep='',end='')
      print("->",end='')
      print(dictr[int(a.arg(1).arg(1))]+"("+dictg[int(a.arg(1).arg(0))]+")",end='')
      print(": ",a.arg(2),end='')
  print()
